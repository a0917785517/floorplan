#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""倉庫平面圖限制驗證器 (validate.py)。
依 CLAUDE.md 第 8 章的限制 + warehouse.yaml 規格自動把關。
有 FAIL 以 exit code 1 結束,不得 commit/push。
用法: python3 validate.py [warehouse-8f.drawio] [--config warehouse.yaml]

事實來源:warehouse.yaml 供給所有期望尺寸/門位/走道寬/固定物/品項;drawio 為受測物。
除既有限制外,另做「規格↔圖」交叉檢查(品項數、●數、門 px、固定物 px),抓「生成出合法但不符規格」。
"""
import sys, re, math
...
F = argv[0] if argv else 'warehouse-8f.drawio'
...
try:
    ...
    print(f'[warn] 未載入 {CONFIG}({e});改用內建預設值,略過規格交叉檢查。')
...
def cfg_fixed(idv):
    ...
    for o in cfg['fixed_objects']:
        if o['id'] == idv: return o
    return None

def gs(s, k, d=None):
    m = re.search(rf'{k}=([^;]+)', s or '')
    v = m.group(1) if m else d
    # 新版 draw.io 主題色:light-dark(#亮, #暗) → 取亮色;'default' 視為未指定
    if isinstance(v, str) and v.startswith('light-dark('):
        mm = re.search(r'light-dark\(\s*(#[0-9A-Fa-f]+)', v); v = mm.group(1) if mm else v
    return v
def clean(v):
...
if walls and (back or front):
    ...
    APPROACH = cfg['doors']['approach_px'] if cfg else 150  # 門前淨空作業區深度(px)≈155cm
...
# 8.4: 門在南牆上的精確位置(沿牆量測),期望值由 warehouse.yaml 供給
DB = cfg['doors']['back'] if cfg else {'width_cm': 180, 'from_left_cm': 912}
DF = cfg['doors']['front'] if cfg else {'width_cm': 180, 'from_right_cm': 125}
if walls:
    W = walls[0]; wleft = W['x']; wright = W['x'] + W['w']
    for d in back:
        w_cm = d['w']/PXCM; left_cm = (d['x']-wleft)/PXCM
        check(abs(w_cm-DB['width_cm']) <= 8, f'後門寬度 = {w_cm:.0f}cm (需 {DB["width_cm"]})')
        check(abs(left_cm-DB['from_left_cm']) <= 10, f'後門左端距左牆 = {left_cm:.0f}cm (需 {DB["from_left_cm"]})')
    for d in front:
        w_cm = d['w']/PXCM; right_cm = (wright-(d['x']+d['w']))/PXCM
        check(abs(w_cm-DF['width_cm']) <= 8, f'前門寬度 = {w_cm:.0f}cm (需 {DF["width_cm"]})')
        check(abs(right_cm-DF['from_right_cm']) <= 10, f'前門右端距右牆 = {right_cm:.0f}cm (需 {DF["from_right_cm"]})')
...
# B01(固定):期望尺寸由 warehouse.yaml 供給(暫定 ≈898×289cm) — 待確認實際尺寸
_b01c = cfg_fixed('B01')
EXPECT_B01 = (_b01c['w'], _b01c['h']) if _b01c else (866, 278)   # px(來自 yaml)
...
# 置物架:期望尺寸由 warehouse.yaml 供給(暫定 ≈70×149cm) — 待確認實際尺寸
shelf = [c for c in cells if '置物' in c['v']]
_shc = cfg_fixed('shelf')
EXPECT_SHELF = (_shc['w'], _shc['h']) if _shc else (67, 144)  # px(來自 yaml)
...
# === 規格↔圖 交叉檢查(warehouse.yaml ↔ drawio):抓「生成合法但不符規格」 ===
if cfg:
    items = cfg['products']['items']
    spec_total = sum(p['count'] for p in items)
    spec_partial = sum(p['partial'] for p in items)
    spec_names = set(p['name'] for p in items)
    # 貨總數 == 規格 Σcount(抓漏/多板漂移,如曾 128→150)
    check(len(goods) == spec_total, f'[交叉]貨總數 == 規格: 圖{len(goods)} / 規格{spec_total}')
    # 逐 SKU 名稱→數量相符
    dgoods = defaultdict(int)
    for c in goods: dgoods[c['v']] += 1
    cnt_mismatch = [(p['name'], dgoods.get(p['name'], 0), p['count'])
                    for p in items if dgoods.get(p['name'], 0) != p['count']]
    check(not cnt_mismatch,
          f'[交叉]逐SKU數量相符: {len(items)} 品項' +
          ('' if not cnt_mismatch else ' → 不符 ' + ', '.join(f'{n}(圖{a}≠規{e})' for n, a, e in cnt_mismatch[:5])))
    # 圖上品名須為規格子集(抓誤分類 / 未知品名)
    extra = sorted(set(dgoods) - spec_names)
    check(not extra, f'[交叉]圖上品名⊆規格品名: {"OK" if not extra else "未知 "+", ".join(extra[:5])}')
    # ● 總數 == 規格 Σpartial
    check(len(dots) == spec_partial, f'[交叉]●總數 == 規格Σpartial: 圖{len(dots)} / 規格{spec_partial}')
    # 門 px 位置 == 由 cfg 反算
    _wl = cfg['wall']['x_px']; _wr = cfg['wall']['x_px'] + cfg['wall']['w_px']
    exp_back = _wl + DB['from_left_cm']*PXCM
    exp_front = _wr - (DF['from_right_cm']+DF['width_cm'])*PXCM
    if back: check(abs(back[0]['x']-exp_back) <= 10, f'[交叉]後門 x == 反算: 圖{back[0]["x"]:.0f} / 算{exp_back:.0f}')
    if front: check(abs(front[0]['x']-exp_front) <= 10, f'[交叉]前門 x == 反算: 圖{front[0]["x"]:.0f} / 算{exp_front:.0f}')
    # 固定物 px == cfg(座標不可更動;以幾何比對,不靠標籤字串)
    fx_bad = []
    for o in cfg['fixed_objects']:
        hit = [c for c in cells if abs(c['x']-o['x']) <= 3 and abs(c['y']-o['y']) <= 3
               and abs(c['w']-o['w']) <= 4 and abs(c['h']-o['h']) <= 4]
        if not hit: fx_bad.append(o['id'])
    check(not fx_bad, f'[交叉]固定物座標 == 規格: {len(cfg["fixed_objects"])} 個' +
          ('' if not fx_bad else ' → 位移/缺 ' + ', '.join(fx_bad[:5])))
    # 規格自洽:成品目標達成
    check(len(prod) == cfg['finished']['target_boards'],
          f'[交叉]成品數 == 規格target: 圖{len(prod)} / 規格{cfg["finished"]["target_boards"]}')

# --- 高頻料靠近後門(warning,不擋 commit) ---
FAST = set(cfg['products']['fast_fills']) if cfg else {'#FFEDD5', '#FCE7F3', '#F3E8FF', '#FDE2E2', '#E5E7EB'}
warns = []
if back:
